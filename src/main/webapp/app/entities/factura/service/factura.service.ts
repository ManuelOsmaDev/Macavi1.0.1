...
export class FacturaService {
  ...
  protected convertDateFromClient(factura: IFactura): IFactura {
    return Object.assign({}, factura, {
      fechaFact: factura.fechaFact?.isValid() ? factura.fechaFact.toJSON() : undefined,
      fechaVenc: factura.fechaVenc?.isValid() ? factura.fechaVenc.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.fechaFact = res.body.fechaFact ? dayjs(res.body.fechaFact) : undefined;
      res.body.fechaVenc = res.body.fechaVenc ? dayjs(res.body.fechaVenc) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((factura: IFactura) => {
        factura.fechaFact = factura.fechaFact ? dayjs(factura.fechaFact) : undefined;
        factura.fechaVenc = factura.fechaVenc ? dayjs(factura.fechaVenc) : undefined;
      });
    }
    return res;
  }
